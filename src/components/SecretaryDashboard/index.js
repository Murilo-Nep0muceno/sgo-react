...
import { FiCheckCircle, FiXCircle, FiClock, FiActivity, FiCalendar, FiUser, FiFilter } from 'react-icons/fi';
    ...
    <div className={styles.modalOverlay} onClick={handleClose}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()} role="alertdialog" aria-modal="true">
        <div className={styles.modalHeader}>
          <h3>{title}</h3>
          <button onClick={handleClose} className={styles.modalCloseButton} disabled={isLoading}>×</button>
        </div>
        ...
        <div key={date} className={styles.dateGroup}>
          ...
          <div className={styles.schedulesGrid}>
            {Array.isArray(groupedSchedules[date]) && groupedSchedules[date].map(schedule => {
              const bookedApps = (schedule.appointments || [])
                .filter(app => app.status === 'agendado' || app.status === 'concluído')
...
const AppointmentForm = ({ schedule, onSubmit, onBack, isLoading, serverError }) => {
  ...
  const handleSubmit = (e) => {
    ...
    let formattedStartTime = formData.get('startTime');
    let formattedEndTime = formData.get('endTime');
    
    if (formattedStartTime && formattedStartTime.length === 5) formattedStartTime += ':00';
    if (formattedEndTime && formattedEndTime.length === 5) formattedEndTime += ':00';
  ...
  return (
    <div>
      ...
      <div className={styles.scheduleCardInfo} role="region">
        <p><strong>Doutor(a):</strong> {schedule?.doctor?.username || 'N/A'}</p>
...
const BookedAppointmentsList = ({ appointments, onReschedule, onCancel, isLoading }) => {
  if (!Array.isArray(appointments) || appointments.length === 0) { 
    return <div className={styles.emptyState}><p>Nenhuma consulta encontrada com os filtros atuais.</p></div>; 
  }
        ...
        <tbody>
          {appointments.map(app => {
            ...
            <tr key={app?.id} className={app?.status === 'cancelado' ? styles.cancelledRow : ''}>
              ...
              <td data-label="Status"><span className={styles[`status-${app?.status}`]}>{app?.status || 'N/A'}</span></td>
...
const AlterarEditForm = ({ appointment, newlySelectedSchedule, onSubmit, onCancel, onGoToSelectSlot, onOpenConfirm, isLoading, serverError }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const finalStartTime = startTime.length === 5 ? startTime + ':00' : startTime;
    const finalEndTime = endTime.length === 5 ? endTime + ':00' : endTime;
...
const SvgPieChart = ({ data }) => {
  ...
  return (
    <div className={styles.chartWrapper}>
      <svg viewBox="-1 -1 2 2" className={styles.svgPie}>
        {data.map((slice, index) => {
          ...
          if (slicePercent === 1) {
            return (
              <g key={index}>
                <circle cx="0" cy="0" r="1" fill={slice.color} className={styles.chartSlice} />
                <text 
                  x="0" 
                  y="0" 
          ...
          const endPercent = cumulativePercent + slicePercent - 0.25;
          ...
          const textX = midX * 0.7; 
          const textY = midY * 0.7;

          const percentage = Math.round(slicePercent * 100);

          cumulativePercent += slicePercent;

          return (
            <g key={index}>
              <path d={pathData} fill={slice.color} className={styles.chartSlice} />
              {percentage >= 5 && ( 
                <text 
                  x={textX} 
                  y={textY} 
...
const ViewReports = ({ token, setNotification }) => {
  ...
  const chartData = useMemo(() => [
    { label: 'Concluídas', value: stats.completed, color: '#28a745' },
    { label: 'Canceladas', value: stats.cancelled, color: '#dc3545' },
    { label: 'Agendadas', value: stats.scheduled, color: '#007bff' }
  ], [stats]);
        ...
        <div>
          <div className={styles.statsCards}>
            ...
            <div className={styles.statCard}>
              ...
              <div className={styles.statInfo}>
                  <h4>Canceladas</h4>
                  <p className={styles.statNumber}>{stats.cancelled}</p>
              </div>
            </div>
            <div className={styles.statCard}>
              <div className={`${styles.statIconBox} ${styles.iconCyan}`}>
                  <FiClock />
              </div>
              <div className={styles.statInfo}>
                  <h4>Agendadas</h4>
          ...
          <div className={styles.reportContentGrid}>
            <div className={styles.dashboardCard}>
                <h3 className={styles.sectionTitle}>Distribuição de Status</h3>
                <div className={styles.chartContainer}>
                    <div className={styles.chartContent}>
                        ...
                        {chartData.map((item, index) => (
                            ...
                            <div className={styles.colorDot} style={{background: item.color}}></div>
            ...
            <div className={styles.dashboardCard}>
                <h3 className={styles.sectionTitle}>Detalhamento</h3>
                <div className={styles.tableContainer}>
                <table className={styles.userTable}>
                    ...
                    {filteredAppointments.map(app => (
                        ...
                        <td data-label="Status"><span className={styles[`status-${app.status}`]}>{app.status}</span></td>
                        </tr>
                    ))}
...
const ViewCreateAppointment = ({ token, setNotification, onAppointmentCreated }) => {
  ...
  const filteredAvailableSchedules = useMemo(() => {
    return allSchedules.filter(schedule => {
      ...
      if (!isFutureOrToday(scheduleDate)) return false;
