...
/**
 * [V30.1] Helper para obter a data de hoje no formato YYYY-MM-DD.
 */
...
// ========================================================================
// === 1. SUB-COMPONENTES "BURROS" (Dumb Components) ======================
// ========================================================================
...
// --- Modal de Confirmação (V6) ---
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message, isLoading }) => {
    const [confirmationText, setConfirmationText] = useState('');
    const isConfirmed = confirmationText.toLowerCase() === 'confirmar';
    const handleSubmit = (e) => { e.preventDefault(); if (isConfirmed) { onConfirm(); } };
    const handleClose = () => { if (!isLoading) { setConfirmationText(''); onClose(); } };
        ...
        <div className={styles.modalOverlay} onClick={handleClose}>
            <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                <div className={styles.modalHeader}>
                    <h3>{title}</h3>
                    <button onClick={handleClose} className={styles.modalCloseButton} disabled={isLoading}>&times;</button>
                </div>
                <form onSubmit={handleSubmit}>
                    <div className={styles.modalBody}>
                        ...
                        <input id="confirmInput" type="text" value={confirmationText} onChange={(e) => setConfirmationText(e.target.value)} placeholder="confirmar" className={styles.modalConfirmInput} autoFocus />
                    </div>
...
// --- Filtros de Agenda (V1) ---
const ScheduleFilters = ({ doctors, onDateChange, onDoctorChange, onClearFilters }) => (
    <div className={styles.filterContainer}>
        <input type="date" onChange={(e) => onDateChange(e.target.value)} className={styles.filterInput} aria-label="Filtrar por data" />
        <select onChange={(e) => onDoctorChange(e.target.value)} className={styles.filterInput} aria-label="Filtrar por dentista">
            <option value="">Todos os Dentistas</option>
            {Array.isArray(doctors) && doctors.map(doctor => (<option key={doctor?.id} value={doctor?.id}>{doctor?.username || 'Doutor Inválido'}</option>))}
        </select>
        ...
        <div>
            {dates.map(date => (
                <div key={date} className={styles.dateGroup}>
                    <h3>{date ? new Date(`${date}T00:00:00Z`).toLocaleDateString('pt-BR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' }) : 'Data Inválida'}</h3>
                    <div className={styles.schedulesGrid}>
                        {Array.isArray(groupedSchedules[date]) && groupedSchedules[date].map(schedule => {
                            ...
                            return (
                                <div key={schedule?.id} className={`${styles.scheduleCard} ${selectable ? styles.selectableCard : ''}`} onClick={() => selectable && schedule && onSelectSchedule(schedule)} role="button" tabIndex={selectable ? 0 : -1} onKeyPress={(e) => selectable && schedule && (e.key === 'Enter' || e.key === ' ') && onSelectSchedule(schedule)}>
                                        ...
                                        <div className={styles.bookedTimes}>
                                            ...
                                            {bookedApps.length > 0 ? (
                                                <ul className={styles.bookedTimesList}>
                                                    {bookedApps.map((app, index) => (<li key={index} className={styles.bookedTimeItem}>{app.startTime?.slice(0, 5)} - {app.endTime?.slice(0, 5)}</li>))}
                                                </ul>
...
// --- Formulário de Agendamento (V20.2) ---
const AppointmentForm = ({ schedule, onSubmit, onBack, isLoading, serverError }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const appointmentData = { email: formData.get('email'), startTime: formData.get('startTime'), endTime: formData.get('endTime'), descricao: formData.get('descricao'), status: 'agendado', };
    ...
    return (
        <div>
            ...
            <div className={styles.scheduleCardInfo}><p><strong>Doutor(a):</strong> {schedule?.doctor?.username || 'N/A'}</p><p><strong>Data:</strong> {scheduleDate}</p><p><strong>Turno Completo:</strong> {scheduleStartTime} - {scheduleEndTime}</p></div>
            <form onSubmit={handleSubmit} className={styles.appointmentForm}>
                <label htmlFor="apt-email">Email do Paciente</label>
                <input id="apt-email" name="email" type="email" placeholder="email@paciente.com" required className={styles.input}/>
...
// --- Lista de Agendados (V5) ---
const BookedAppointmentsList = ({ appointments, onReschedule, onCancel, isLoading }) => {
    if (!Array.isArray(appointments) || appointments.length === 0) { return <p>Nenhuma consulta agendada encontrada.</p>; }
    return (
        <div className={styles.tableContainer}>
            <table className={styles.userTable}>
                <thead><tr><th>Paciente</th><th>Doutor(a)</th><th>Data</th><th>Horário</th><th>Status</th><th>Descrição</th><th>Ações</th></tr></thead>
                <tbody>{appointments.map(app => (
                    <tr key={app?.id} className={app?.status === 'cancelado' ? styles.cancelledRow : ''}>
                        ...
                        <td data-label="Ações">
                            <button onClick={() => app && onReschedule(app)} className={`${styles.actionButton} ${styles.editButton}`} disabled={isLoading || app?.status === 'cancelado' || app?.status === 'concluído'}>Alterar</button>
                            <button onClick={() => app?.id && onCancel(app.id)} className={`${styles.actionButton} ${styles.deleteButton}`} disabled={isLoading || app?.status === 'cancelado' || app?.status === 'concluído'}>Cancelar</button>
                        </td>
                    </tr>
                ))}</tbody>
            </table>
...
const AlterarEditForm = ({ appointment, newlySelectedSchedule, onSubmit, onCancel, onGoToSelectSlot, onOpenConfirm, isLoading, serverError }) => {
    // [V30.1] Adiciona fallbacks para garantir que o estado NUNCA seja nulo.
    ...
    useEffect(() => {
        if (newlySelectedSchedule) {
            ...
            setDate(newlySelectedSchedule.dayOfWeek || getTodayString()); // Adiciona fallback
            setStartTime(newMin);
            setEndTime(newMax); 
    ...
    return (
        <div>
            <button onClick={onCancel} className={styles.backButton}>&larr; Cancelar Alterações</button>
            <h4>Editar / Alterar Consulta</h4>
            <div className={styles.scheduleCardInfo}><p><strong>Paciente:</strong> {patientName}</p><p><strong>Doutor(a):</strong> {doctorName}</p></div>
            <form onSubmit={handleSubmit} className={styles.appointmentForm}>
...
// ========================================================================
// === 2. COMPONENTES DE VISÃO "INTELIGENTES" (Smart Components) ==========
// ========================================================================

// --- Visão: Gerenciar Consultas Agendadas (V30) ---
const ViewManageAppointments = ({ token, setNotification, onStartAlter }) => {
    ...
    const handleCancelAppointment = (appointmentId) => {
        ...
        setModalState({ isOpen: true, title: 'Confirmar Cancelamento', message: 'Você tem certeza que deseja CANCELAR esta consulta? Esta ação não pode ser desfeita.', onConfirm: () => executeCancelAppointment(appointmentId) });
    };
    ...
    return (
        <div>
            ...
            <BookedAppointmentsList appointments={bookedAppointments} onReschedule={onStartAlter} onCancel={handleCancelAppointment} isLoading={isLoading || isLoadingCancel} />
            <ConfirmModal 
                isOpen={modalState.isOpen} 
                onClose={closeModal} 
                onConfirm={modalState.onConfirm} // [V30.1] Corrigido (já estava correto nesta view)
...
// --- Visão: Cadastrar Novo Paciente (V30) ---
const ViewRegisterClient = ({ setNotification, onClientCreated }) => {
        ...
        <div>
            <CreateUserForm
                ...
                setNotification={(error) => {
                    const friendlyError = translateErrorMessage(error.message, "Falha ao criar paciente.");
...
// --- Visão: Criar Novo Agendamento (V30) ---
const ViewCreateAppointment = ({ token, setNotification, onAppointmentCreated }) => {
    ...
    if (selectedSchedule) {
        return <AppointmentForm schedule={selectedSchedule} onSubmit={handleCreateAppointmentSubmit} onBack={() => { setSelectedSchedule(null); setServerError(null); }} isLoading={isSubmitting} serverError={serverError} />;
    }
    return (
        <div>
            <h3>Agendar Nova Consulta</h3><p>Selecione uma vaga disponível abaixo.</p>
            ...
            <SchedulesList groupedSchedules={groupedAvailableSchedules} onSelectSchedule={schedule => { setSelectedSchedule(schedule); setServerError(null); }} selectable={true} />
        </div>
    );
};

// --- Visão: Alterar Consulta (V30.1) ---
const ViewAlterAppointment = ({ token, setNotification, appointment, onCancelAlter, onAlterSuccess }) => {
    ...
    const handleOpenAlterarConfirm = (updatedDataFromForm) => {
        ...
        setModalState({ 
            ...
            onConfirm: () => executeAlterarSubmit(updatedDataFromForm) // Armazena a função COM os dados
        });
    };
    ...
    if (internalView === 'selectSlot') {
        return (
            <div>
                <button onClick={() => setInternalView('editDetails')} className={styles.backButton}>&larr; Voltar para Edição</button>
                <h3>Selecionar Nova Vaga</h3>
                ...
                <SchedulesList groupedSchedules={groupedAvailableSchedules} onSelectSchedule={handleSelectScheduleForAlterar} selectable={true} />
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <ConfirmModal 
                isOpen={modalState.isOpen} 
                onClose={closeModal} 
                onConfirm={modalState.onConfirm} // [V30.1 CORREÇÃO] Chama a função guardada no estado
                title={modalState.title} 
                message={modalState.message} 
                isLoading={isSubmitting} 
            />
...
// ========================================================================
// === 3. COMPONENTE PRINCIPAL (ORQUESTRADOR V30) =========================
// ========================================================================
const SecretaryDashboard = () => {
    ...
    // --- Renderizador de Visão Ativa ---
    const renderActiveView = () => {
        switch (activeView) {
            case 'registerClient':
                return (<ViewRegisterClient setNotification={setNotification} onClientCreated={handleClientCreated} />);
            ...
            case 'alterAppointment':
                if (!appointmentToAlter) { handleCancelAlter(); return <p>Erro...</p>; }
